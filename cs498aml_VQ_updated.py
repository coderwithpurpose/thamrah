...
class acc_classifier:
    ...
    def cluster_data(self, num_clusters_1st_lvl=30, num_clusters_2nd_lvl=9):
    ...
    def quantize_data(self, data, num_clusters_1st_lvl=30, num_clusters_2nd_lvl=9):
...
# plotting bar graphs for each mean histograms

import matplotlib.pyplot as plt

mean_hists = np.zeros((14,270))
print(np.shape(hists_train))
i = 0
x = range(1,271,1)

for act in dict2.keys():
    hist_temp = dict2[act]
    print(np.shape(hist_temp), "plotting histograms")
    mean_hists[i,:] = np.mean(hist_temp, axis = 0)
    plt.subplot(2, 7, i + 1)
    plt.plot(x, mean_hists[i])
    plt.title(act)
    i+=1


print(len(mean_hists[0]))
print(len(mean_hists[1]))
...
print("training data : ", np.shape(hists_train), np.shape(labels_train))
print("testing data : ", np.shape(hists_test), np.shape(labels_test))
...
plt.figure()
plot_confusion_matrix(cnf_matrix, classes=temp_labels_train, normalize=True,
